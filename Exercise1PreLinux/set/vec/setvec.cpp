...
/* ************************************************************************** */
    ...
    template <typename Data> SetVec<Data>& SetVec<Data>::operator=(const SetVec<Data>& vec){ 
        Vector<Data>::operator=(vec); 
        return *this;
    }

    template <typename Data> SetVec<Data>& SetVec<Data>::operator=(SetVec<Data>&& vec) noexcept{ 
...
//SetVec è un vettore circolare e ordinato (creascente). Ho aggiunto un controllo per evitare eliminazioni errate.
    ...
    template <typename Data> Data SetVec<Data>::PredecessorNRemove(const Data& val){ 
    ...
    template <typename Data> void SetVec<Data>::RemovePredecessor(const Data& val){ 
        try{ 
            if(val == Min()) 
                throw std::length_error("You are looking for the predecessor of the min, news for you: it doesn't exist!"); 
        }catch(const std::logic_error& e){ 
    ...
    template <typename Data> Data SetVec<Data>::SuccessorNRemove(const Data & val){ 
        try{ 
            if(val == Max())
                throw std::length_error("You are looking for the predecessor of the max, news for you: it doesn't exist!"); 
        }catch(const std::logic_error& e){ 
    ...
    template <typename Data> void SetVec<Data>::RemoveSuccessor(const Data& val){ 
        try{ 
            if(val == Max())
                throw std::length_error("You are looking for the predecessor of the max, news for you: it doesn't exist!"); 
        }catch(const std::logic_error& e){ 
